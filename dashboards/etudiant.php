                ...
                <div style="display:flex;gap:.75rem;flex-shrink:0;align-items:center;">
                    <?php if ($mod['cert_num']): ?>
                        <a href="/actions/download_certificate.php?module_id=<?= $mod['id'] ?>" 
                           class="btn btn-filled" style="padding:.5rem 1rem;font-size:.85rem;">
                           🏆 Certificat
                        </a>
                    <?php endif; ?>
            ...
            <div id="moduleContent_<?= $mod['id'] ?>" style="display:none;margin-top:1.5rem;">
                ...
                ?>
                    <div style="margin-bottom:1.5rem;">
                        <h4 style="color:var(--color-accent);font-size:.9rem;margin-bottom:.75rem;border-bottom:1px solid rgba(255,255,255,.05);padding-bottom:.5rem;">
                        ...
                        <div style="border:1px solid rgba(255,255,255,.06);border-radius:10px;padding:1rem;margin-bottom:.75rem;background:rgba(0,0,0,.1);">
                            <div style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:.75rem;margin-bottom:.75rem;">
                                ...
                                <div style="display:flex;gap:.5rem;flex-wrap:wrap;flex-shrink:0;">
                                    <?php if ($l['pdf_path']): ?>
                                        <a href="/<?= htmlspecialchars($l['pdf_path']) ?>" target="_blank" class="btn btn-secondary" style="padding:.35rem .75rem;font-size:.78rem;">📄 PDF</a>
                                    <?php endif; ?>
                                    <?php if ($l['video_path']): ?>
                                        <button class="btn btn-secondary" style="padding:.35rem .75rem;font-size:.78rem;"
                                            onclick="openVideoPlayer('<?= htmlspecialchars($l['video_path']) ?>', '<?= htmlspecialchars(addslashes($l['title'])) ?>')">🎬 Vidéo</button>
                                    <?php endif; ?>
                                    <?php if (!empty($l['eval_id']) && !empty($quizData[$l['eval_id']])): ?>
                                        <button class="btn btn-purple" style="padding:.35rem .75rem;font-size:.78rem;"
                                            onclick="launchQuiz(<?= $l['eval_id'] ?>, <?= $mod['id'] ?>)">
                                            <?= ($lessonPct > 0) ? '🔁 Retenter' : '✏️ Évaluation' ?>
                                        </button>
                                    <?php endif; ?>
                                </div>
                            </div>
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:1.25rem;">
        ...
        <div class="glass-card" style="padding:1.5rem;border-radius:var(--border-radius-md);">
            <div style="display:flex;align-items:flex-start;justify-content:space-between;margin-bottom:1rem;">
                <div style="width:44px;height:44px;background:rgba(100,255,218,.1);border-radius:10px;display:flex;align-items:center;justify-content:center;font-size:1.3rem;flex-shrink:0;">📘</div>
